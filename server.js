...
const FFT_SRC = {
  // [2026-08-16] MS PASSE SUR UNE FIFO. Un cfile est le mauvais objet pour une
  // vue live : fftReadTail lit la queue d'apres statSync().size, donc tout
  // producteur qui reboucle ou tronque fait s'effondrer la taille et vide la
  // pane. fftReadLive, lui, ouvre la fifo en O_RDWR|O_NONBLOCK (jamais d'EOF),
  // garde un tampon glissant borne, et CREE la fifo si elle manque — la pane
...
// [2026-08-16] RESOLUTION DYNAMIQUE DE LA SOURCE.
// Le probleme : un cfile qui REBOUCLE est le mauvais objet pour fftReadTail, qui
// lit la queue d'apres statSync().size. A chaque rebouclage la taille s'effondre,
// nbytes tombe sous FFT_NFFT*8, la fonction rend null et la pane se vide ; autour
// du point de bouclage elle lit en plus un melange incoherent. C'est ce que
// l'operateur voyait « peter » sur /dev/shm/dsp_iq.cfile toutes les ~29 s.
// La FIFO est le bon objet : fftReadLive l'ouvre en O_RDWR|O_NONBLOCK (donc
// jamais d'EOF), garde un tampon glissant borne, et la cree au besoin.
...
// même erreur de nature : on affichait un état de TRAME là où l'opérateur attend
// l'état de la SESSION.
//   1. `mode` était lu dans l'environnement de `calypso-ipc-device`, qui
//      N'EXISTE PAS en mode CALYPSO_BRIDGE=pont (start-direct.sh pose
//      SKIP_IPC_DEVICE=1). Le pgrep échouait, `mode` restait '?', et le badge ne
//      voyait donc JAMAIS l'a5/1 dans ce mode.
//   2. `algo`/`active` étaient recalculés toutes les 3 s en RELISANT
//      /dev/shm/calypso_kc. Ce fichier est transitoire par construction : purgé
//      au démarrage du pont, effacé au teardown, et absent tant qu'aucun
//      CIPHERING MODE COMMAND n'est passé. Le badge suivait donc la présence
//      d'une clé À UN INSTANT — d'où la bascule A5/0 ↔ A5/1 en cours de session.
// Désormais : le mode vient de l'AUTORITÉ (la directive `encryption` d'osmo-bsc,
// c'est elle qui décide de ce qui se passe sur l'air), et « une clé a été vue »
// est VERROUILLÉ pour la durée du run. Le verrou n'est relâché qu'au changement
// de run, détecté par le PID d'osmo-bsc — sinon un badge « A5/1 vu » survivrait
// à une relance en a5/0 et mentirait dans l'autre sens.
var A5_BSC_CFG = process.env.OSMO_BSC_CFG || '/etc/osmocom/osmo-bsc.cfg';
var a5Latch = { run: null, seen: false, algo: 0, mode: '?' };

function a5StackRunId() {
  // Identité du run : le PID d'osmo-bsc change à chaque relance de la pile.
  try { return execFileSync('pgrep', ['-f', 'osmo-bsc -c']).toString().split('\n')[0].trim(); }
  catch (e) { return ''; }
}

function a5SessionMode() {
  // 1) L'autorité : ce que le BSC impose réellement sur l'air.
  try {
    var m = fs.readFileSync(A5_BSC_CFG, 'utf8').match(/^[ \t]*encryption[ \t]+a5[ \t]+([0-9 \t]+)/m);
    if (m) return 'a5 ' + m[1].trim().replace(/[ \t]+/g, ' ');
  } catch (e) {}
  // 2) Secours : ENCRYPTION= dans l'environnement d'un process de la pile. La
  //    liste couvre les modes où tel ou tel composant n'existe pas — l'ancien
  //    code n'essayait QUE calypso-ipc-device, absent en mode pont.
  var cands = ['calypso-ipc-device -u', 'pont/pont.py', 'osmo-bts-trx',
               'qemu-system-arm.*calypso', 'run.sh'];
  for (var c = 0; c < cands.length; c++) {
    try {
      var pid = execFileSync('pgrep', ['-f', cands[c]]).toString().split('\n')[0].trim();
      if (!pid) continue;
      var env = fs.readFileSync('/proc/' + pid + '/environ', 'utf8').split('\0');
      for (var i = 0; i < env.length; i++)
        if (env[i].indexOf('ENCRYPTION=') === 0) return env[i].slice(11);
    } catch (e) {}
  }
  return '?';
}
...
function readA5Status() {
  var run = a5StackRunId();
  if (run !== a5Latch.run) {
    // Nouveau run : on relâche le verrou ET le cache de clé, sinon la clé du run
  ...
  var mAlgo = /a5\D*([1-3])/.exec(a5Latch.mode);
...
wss.on('connection', function(ws, req) {
  ...
  ws.on('message', function(raw, isBinary) {
    ...
    switch (msg.type) {
      ...
      // [2026-08-16] `kc` etait JETE ici : l'interface l'envoyait bien
      // (index.html toggleRec -> kc: document.getElementById('rec-kc').value)
      // mais il n'entrait pas dans opts, donc rec.js retombait toujours sur le
      // Kc capture automatiquement et le champ de saisie n'avait AUCUN effet.
      case 'record_start':           recorder.start(msg.source, { m: msg.m, t: msg.t, kc: msg.kc }, ws); break;
